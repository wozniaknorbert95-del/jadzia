...
def _mock_breaker():
    """Return a MagicMock that behaves like a closed circuit breaker."""
    breaker = MagicMock()
    breaker.is_call_permitted.return_value = True
...
def test_read_file_calls_validate_operation_and_read_file_ssh():
    ...
    with patch("agent.tools.ssh_orchestrator.get_breaker", return_value=_mock_breaker()):
