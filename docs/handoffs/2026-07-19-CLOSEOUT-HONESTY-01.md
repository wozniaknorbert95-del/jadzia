...
|----|-----------------------------|---------|----------|
| **D1 Tip SoT** | `todo.json` + MKT-SHIP note + VCMS handbook tip = jadzia **`8d40efc`** (nie `3e60437`) | **PASS** (po push/deploy VCMS w tej sesji) | tip VPS `git rev-parse` = `8d40efc` |
...
| **D6 VPS hygiene** | Brak `?? path` w `/opt/jadzia`; tip nadal `8d40efc` | **PASS** gdy usunięte | plik był typescript `script` 2026-07-18 |
