...
| Łańcuch ICP→CF→Val→Publish | `done` (tool) | `hub agents flow` — pełny chain, publish gated |
...
| MCP: GA4 | `waiting` | fail-closed stub `done`; live = brak pliku SA (credentials) |
...
1. GA4 live — plik SA na VPS + `DEMAND_OS_GA4_LIVE=1` (GO).
2. GDrive real connector (zastąpić stub) — bez GO na network.
3. Desk tile „Demand OS Agenci” z `list_agents()` (GO na UI diff).
