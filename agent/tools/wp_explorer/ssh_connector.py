...
def _force_wp_explorer_logging() -> None:
    ...
    try:
        ...
        if not log_file:
            log_file = (
                "/root/jadzia/logs/wp_explorer.log" if os.name != "nt" else "logs/wp_explorer.log"
            )
...
class SSHConnector:
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(1, self.config.ssh_retry_count + 1):
            try:
                ...
                verify_host_key_fingerprint(self._client)
    ...
    async def download_directory_as_tar(self, remote_path: str, local_path: Path) -> DownloadResult:
        """
        ...
        try:
            ...
            logger.debug(
                "[WP_EXPLORER] Tar result: exit=%s, stderr=%s",
                result.exit_code,
                result.stderr[:200],
            )
            ...
            for attempt in range(1, self.config.ssh_retry_count + 1):
                try:
                    ...
                    last_exc = e
                    logger.exception(
                        "SFTP download attempt %d/%d failed",
                        attempt,
                        self.config.ssh_retry_count,
                    )
