...
ALLOWED_TELEGRAM_USERS = set(
    user_id.strip() for user_id in ALLOWED_TELEGRAM_USERS_RAW.split(",") if user_id.strip()
)
...
class TelegramWebhookRequest(BaseModel):
    ...
    callback_data: Optional[str] = Field(
        None, description="Inline keyboard callback (e.g. task_id:approve:yes)"
    )
...
def validate_webhook_secret(x_webhook_secret: Optional[str] = Header(None)) -> bool:
    ...
    if not hmac.compare_digest(x_webhook_secret, TELEGRAM_WEBHOOK_SECRET):
        raise HTTPException(status_code=401, detail="Invalid webhook secret")

    return True


def validate_native_telegram_secret(
    x_telegram_bot_api_secret_token: Optional[str],
) -> bool:
    """Validate Telegram's native webhook secret-token header."""
    if not TELEGRAM_WEBHOOK_SECRET:
        _log.warning("TELEGRAM_WEBHOOK_SECRET not configured!")
        return True
...
def validate_telegram_request(
    request: TelegramWebhookRequest, x_webhook_secret: Optional[str] = Header(None)
