...
async def handle_scan_command(
    ...
    """Obsługa /skanuj — uruchamia WPExplorer (manual scan)."""
    try:
        from ..tools.wp_explorer import WPExplorer

        explorer = WPExplorer()
        result = await explorer.scan_project(source="manual")

        if result.success:
            return (
                "✅ **SKAN ZAKOŃCZONY**\n\n"
                f"- Pliki: {result.file_count}\n"
                f"- Hooki: {result.hook_count} (dynamiczne: {result.dynamic_hook_count})\n"
                f"- Zależności: {result.dependency_count}\n"
                f"- Czas: {result.duration_seconds}s\n"
                f"- Output: {result.structure_path or 'agent/context/project_structure.json'}",
                False,
                None,
            )

        # failure path
        errors = "\n".join(f"- {e}" for e in (result.errors or [])) or "- (brak szczegółów)"
        return (
            "❌ **SKAN NIEUDANY**\n\n"
            f"{errors}",
            False,
        ...
        send_alert("scan_failed", None, str(e))
        return (f"❌ Skanowanie nie powiodło się: {e}", False, None)
...
def handle_help(
    ...
    return (
        ...
        "/skanuj - zeskanuj strukturę projektu WordPress (WPExplorer)\n"
        "/help - ta pomoc\n\n"
        "**Przyklady polecen:**\n"
